...
The service instance is currently running in Microsoft Azure's commercial cloud (region TBD) as it is not yet available in Azure's government cloud offering. The Azure account is owned by the VA Enterprise Cloud (VAEC) team and is identified as the Enterprise Development Environment (EDE) where VAEC users, including users who have not yet obtained a PIV, may sandbox applications. Access to the bot service configuration is controlled by Microsoft Azure's Identity and Access Management (IAM) service.
