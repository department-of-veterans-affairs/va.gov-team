...
1. Begin nvaigating in your web browser to the `/authorize` endpoint generated by Postman. It is critical that the `code_challenge` is the same as Postman's saved variable in order for later token request to work:
...
2. Complete the CSP authentication and find the `callback` call that `vets-api` makes to `vets-website` at the end of the authentication process. This can either be done in your web browser or in Postman via the [Postman Interceptor](https://chrome.google.com/webstore/detail/postman-interceptor/aicmkgpgakddgnaphhhpliifpcfhicfo?hl=en) plugin:

    `http://localhost:3001/auth/login/callback/?code=37349963-776e-4e39-aa86-2369da43b51a&type=logingov`
3. Copy the `code` URL parameter into your Postman SiS collection variables as the new `auth_code` and save the variables file.
4. You should now be able to use Postman to make a `/token` call, passing the `auth_code` and receiving a set of tokens that Postman will save to make further calls. Postman will also automatically update tokens for `/refresh` calls and wipe them for `/logout`, `/revoke`, and `/revoke_all` calls.
