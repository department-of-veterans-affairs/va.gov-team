...
NOTE: As we started doing research, we realized that it would be best to start with a short number of requests in order to understand the fulfullment process.
...
| Percentage of requests | On a monthly basis, we track the percentage of requests that were fulfilled via the Request & Supp
