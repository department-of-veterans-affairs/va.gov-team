...
* Product Description  
   * ```Identity team products cover authentication. We connect sign-in credentials (from ID.me, Login.gov) to both VA.gov and the VA mobile app. We also own the universal sign-in page and modal, Terms of Use page, and all sign in flows from any page on VA.gov, like sign in widgets. Our major initiatives currently include deprecating MHV and DS Logon sign-in methods and working on a delegate access or caregiver access solution```
