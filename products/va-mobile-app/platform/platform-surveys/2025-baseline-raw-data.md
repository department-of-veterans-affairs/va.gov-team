### Raw data from 2025 Mobile Platform Survey
> [analysis is separate](https://github.com/department-of-veterans-affairs/va.gov-team/blob/master/products/va-mobile-app/platform/platform-surveys/2025-baseline-summary.md)

Question 1: How long have you been building on the VAHB app? 
- 14 out of 14 participants answered

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Less than 6 months | 6 | 42.9% | 
| 6 months to 1 year | 7 | 50% | 
| 1 to 2 years | 0 | 0% | 
| More than 2 years | 1 |  7.1%

Question 2: With which practice is your role most closely aligned?
- 14 out of 14 participants answered

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Product| 3| 21.4%
| UX (Design/research)| 5| 35.7%
| Engineering| 6| 42.9%

Question 3: I am comfortable with the type and level of decisions I can make about the feature or experience being built
- 14 out of 14 participants answered

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree | 3 |21.4%
| Somewhat agree | 5 | 35.7%
| Neutral | 5 | 35.7%
| Somewhat disagree | 1 | 7.1%
| Strongly disagree | 0 | 0%

Question 4: 
What do you wish were different about your ability to make decisions? (Multi-line text)
- 12 of 14 participants 

- "Being more informed, know the product more in detail"
- "Our team is unique that we don't own the features we are building, nor havefinal control of the app. Right now it feels like any decision must be approvedby another team and my role is more to manage what decisions have beenmade and help unblock folks that need decisions from other teams."
- "More user research to gather data from to support or inform decisions."
- "Keep consistent patterns"
- "Wish decisions were more data backed than contract backed"
- "I've just begun my work with the Mobile team, and things have been goingsmoothly; I am optimistic about our working relationship."
- "Since MFS is building I would like to see clear timelines and details about the status of their work. It would be great if there were touchpoints/ an approval process where the product team who is responsible for the overall experience across web + mobile are brought up to speed can confirm that the direction is inline with what is live on the web and can resolve any confusion."
- "Do not wish anything different"
- "No wish to make anything different"
- "I've been getting conflicting messaging on what we can/can't change in the app. I've been told the other team owns the content and we're just implementing the product for them on the app, and that content needs to align from most people except Ryan. Additionally, all feedback must be documented in order to act on it, and we've received no written feedback from Core Mobile so far, but have received feedback from governance that contradicts guidance from Core Mobile. I feel like I'm not getting the proper feedback at the right times which has led to confusion and rework."
- "Understanding the why to things are the way they are"
- "that i understood who else i should consult with in order to make the decisions"


Question 5: 
Which of these represents your role?
- 14 out of 14 participants 

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| I am a member of the Core Mobile team | 4 |28.6%
| I am a member of Mobile Feature Support or another experience team which builds mobile features | 4 | 28.6%
| I am a member of an experience team collaborating with the Core Mobile team or Mobile Feature Support team to build a VAHB experience | 6 | 42.9%

Question 6: [Core Mobile specific]
How much of your time is spent on feature work?
- 4 out of 14 participants 

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| 10% or less | 0 | 0%
| 11-25% | 1 | 25%
| 26-50% | 0 | 0%
| 51-75% | 2 | 50%
| More than 75% | 1 |5%

Question 7 : [Core Mobile specific]
How much of your time is spent answering questions or supporting other teams building features on the VAHB app?
- 4 out of 14 participants 

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| 10% or less | 1 | 25%
| 11-25% | 1 | 25%
| 26-50% | 1 | 25%
| 51-75% | 1 | 25%
| More than 75% | 0 |0%

Question 8: [Core Mobile specific]
How much of your time is spent answering questions or supporting other teams building features on the VAHB app?
- 4 out of 14 participants 

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| 10% or less | 3 | 75%
| 11-25% | 0 | 0%
| 26-50% | 0 | 0%
| 51-75% | 1 | 25%
| More than 75% | 0 |0%

Question 9: [Core Mobile specific]
What types of questions do you most often field from other teams? (Multi-select) 
- 4 out of 14 participants 

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Engineering| 3 | 75%
| Design | 2 | 50%
| Feature prioritization/status| 0 | 0%
| Release process | 2 | 50%
| Analytics | 3 | 75%
| Monitoring | 0 |0%
| Documentation-related | 3 | 75%

Question 10:  [Core Mobile specific]
If you field other types of questions, add information about them here (optional)
- 0 out of 14 participants

Question 11:  [Core Mobile specific]
I feel supported in my role on the team
- 4 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |2 |50%
| Somewhat agree | 2 |50%
| Neutral | 0 | 0%
| Somewhat disagree | 0 | 0%
| Strongly disagree | 0 | 0%

Question 12:  [Feature/Experience teams specific]
Did you go through Mobile onboarding?
- 4 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Yes |2 |50%
| No 2 |50%

Question 13:  [Feature/Experience teams specific]
How well did onboarding prepare you to be successful building on the VAHB app?
- 4 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Well | 0 | 0%
| Somewhat well | 25% | 
| Neutral | 3 | 75% 
| Only sllightly 0 | 0%
| Not at all | 0 | 0%

Question 14:  [Feature/Experience teams specific]
What about your onboarding experience contributed to your rating?
- 4 out of 14 participants
- "I don't think at the time of my onboarding I was presented with mobile docs. The only thing I was aware of was the Release schedule."
- "I haven't gone through an onboarding experience since I'm a designer, but I've read the Slack canvas and joined your office hours, which were really productive. The team was welcoming and wonderful."
- "There was some minor confusion on how to get up and running with everything"
- "I came in about a year ago, and a lot of things were out of date. I know there have been lots of improvements, but there's still quite a bit of stuff missing in the documentation."

Question 15:  [Feature/Experience teams specific]
I understand how to engage the Mobile team about a new feature for the VAHB app
- 10 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |3 |30%
| Somewhat agree | 5 |50%
| Neutral | 1 | 10%
| Somewhat disagree | 0 | 0%
| Strongly disagree | 1 | 10%

Question 16:  [Feature/Experience teams specific]
I understand how feature requests are prioritized
- 10 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |0 | 0%
| Somewhat agree | 4 |40%
| Neutral | 3 |30%
| Somewhat disagree | 3 |30%
| Strongly disagree | 0 | 0%

Question 17:  [Feature/Experience teams specific]
I know how to check on the status of a new request or feature in development
- 10 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |1 | 10%
| Somewhat agree | 2 | 20%
| Neutral | 2 | 20%
| Somewhat disagree | 5 |50%
| Strongly disagree | 0 | 0%

Question 18:  [Feature/Experience teams specific]
I understand the steps between feature development and release and what is expected of me/my team during each step.
- 10 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |1 | 10%
| Somewhat agree | 3 | 30%
| Neutral | 2 | 20%
| Somewhat disagree | 3 | 30%
| Strongly disagree | 1 | 10%

Question 19:  [Feature/Experience teams specific]
I understand what is expected of my team related to post-release analytics and monitoring
- 10 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |0 | 0%
| Somewhat agree | 3 | 30%
| Neutral | 3 | 30%
| Somewhat disagree | 1 | 10%
| Strongly disagree | 3 | 30%

Question 20:  [Feature/Experience teams specific]
I understand how to get support from the Mobile team
- 10 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |5 | 50%
| Somewhat agree | 3 | 30%
| Neutral | 1 | 10%
| Somewhat disagree |1 | 10%
| Strongly disagree | 0 | 0%

Question 21:  [Feature/Experience teams specific]
I know how to find Veteran feedback and other data related to my team's feature/experience
- 10 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |0 | 0%
| Somewhat agree | 2 | 20%
| Neutral | 3 | 30%
| Somewhat disagree | 3 | 30%
| Strongly disagree | 2 | 20%

Question 22:  [Feature/Experience teams specific]
The feedback received from the Mobile team is
- 10 out of 14 participants

|  | Very | Mostly | Neutral | Minimally | Not at all |  
| --- | --- | --- |--- | --- |--- | 
| Clear | 10.0% | 60.0% | 20.0% | 10.0% | 0.0%
| Consistent from one interaction with the Mobile team to another | 20.0%| 40.0%| 40.0%| 0.0%| 0.0%
| Consistent with feedback from others outside the Mobile team (i.e., Platform,CoP, or Collab cycle)| 20.0%| 30.0%| 40.0%| 10.0%| 0.0%
| Consistent with documented VAHB guidance| 10.0%| 40.0%| 50.0%| 0.0% | 0.0%

Question 23: 
I can easily self-serve and find answers to my questions
- 14 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |2 | 20%
| Somewhat agree | 7 | 50%
| Neutral | 4 |28.6%
| Somewhat disagree |1  | 7.1%
| Strongly disagree | 0 | 0%

Question 24: 
I understand the criteria used to make decisions about the feature or experience being built
- 14 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |3 | 21.4%
| Somewhat agree | 6 | 42.9%
| Neutral | 4 |28.6%
| Somewhat disagree |1  | 7.1%
| Strongly disagree | 0 | 0%

Question 25: 
I understand the relationship between the Platform collab cycle and my work/feature in the VAHB app
- 14 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |2 | 14.3%
| Somewhat agree  |2 | 14.3%
| Neutral | 5 |35.7%
| Somewhat disagree | 5 |35.7%
| Strongly disagree | 0 | 0%

Question 26: 
I have the knowledge and support I need to deliver a Veteran VAHB app experience consistent with VA standards, mobile team governance, and industry best practices.
- 14 out of 14 participants

| Answer | # of respondents |  Percentage | 
| --- | --- | --- |
| Strongly agree |2 | 14.3%
| Somewhat agree  |8 | 57.1%
| Neutral | 4 |28.6%
| Somewhat disagree | 0 | 0%
| Strongly disagree | 0 | 0%

Question 27: 
What has been the smoothest part of your experience building on the VAHB app?
- 13 out of 14 participants

- "I haven't build on the VAHB app but work closely and collaboratively with the MFS. We have great communication."
- "Core Mobile team members have been great to work with when building features on the app."
- "Building off existing knowledge, processes, or the collaborative nature of the team providing support and direction for a new team member."
- "Whenever features have fully fleshed out AC and no further questions were needed"
- "automated releases"
- "I haven't finished my build yet."
- "The collaboration with MFS Team."
- "mobile team has been good to work with"
- "I am not building on the VAHB app. Just supporting the team"
- "There's lots of code that provides a good example of best practices"
- "It's been great to meet with Ryan and the Core Mobile UX team. The team is friendly and generally responsive in Slack when I ask questions."
- "Getting the dev environment set up"
- "quick comms w/ members of the team"


Question 28: 
What do you wish you had known earlier about building on the VAHB app?
- 13 out of 14 participants

- We got stuck in a loop where we took what the Collab Cycle told us to do,which aligned with best practices for web. When we met with the Core Mobile team UX, they provided different guidance, and so we had to go back and make changes to our design. I wish it were clearer what mobile app standards will differ from the web experience, including accessibility, whenwe began our first design iteration for mobile."
- "its not clear to me how support for the mobile app feature set is to be managed"
- "I wish there were more documentation on best practices for debugging with React Native"
- "All of the processes needed to change something in the design system"
- "Best standards for writing tests"
- "that the documentation site existed"

Question 29: 
If we could improve one thing about your experiencebuilding for the VAHB app, what would it be?
- 9 out of 14 participants

- "Right now, it feels like our team has to go through the collab cycle and then a mobile design process. These processes at times conflict with one another. I wish that we were given feedback from the collab cycle from the Mobile experts, and that we didn't have to manage various feedback and try to sort out which feedback we need to adhere to. One improvement is if Mobile experts were providing feedback on our designs during Design Intent and Midpoint Review, as well as Mobile engineers were providing feedback during Architecture Intent. This could help our team avoid conflicting feedback, as we would expect to get the source of truth from those individuals."
- "More focus on platform related priorities."
- "So far so good!"
- "unified standards and reusable components"
- "perhaps a kickoff info call explaining the features to be build, how long term support will work, and how to manage changes and drift between the tw owould have been nice up front"
- "We need better staging users and documentation for testing so we can test through all scenarios. It would also be nice if you could clone and create our own staging users."
- "Help vetting collab cycle feedback that contradicts mobile's stance and receiving written feedback from Core Mobile throughout the process"
- "Better local api testing support"
- "knowing where to look for for tickets"

Question 30: 
Is there anything else you think we should know?
- 6 out of 14 participants

- "We are working closely with the MFS team to ensure consistency with the desktop VA website"
- "I think it would be helpful to develop processes, such as a product playbook,for the mobile app. All of these items that we are being asked to do on the web should be ideally have a process for mobile. This could also help the Collab Cycle team provide guidance for teams building on the mobile app."
- "Would love to be able to connect a date log of found issues and resolutions deployed to cross reference with app review dates so we can see possible correlations or causation between feedback and found issues."
- "Doing a great job, feel supported all around!"
- "Excited for the future and happy to be a part of it! Happy to help in any way we can."
- "Nope"
