...
# Introduction
In the scenario that we decide on an in-person and hybrid model, we have questions best answered in person with individuals who work at the Blind Rehabilitation Center at Hines hospital. The reasoning behind this has many determining factors, such as the current pandemic and internet connectivity, etc. In our vision, two dominant testing strategies are coming out of discovery.
...
We are interested in testing various ranges of skill levels using accessibility tools. Would this be an option for us? Tools of interest are the following: using a braille keyboard, screen reader, and zoom text.
