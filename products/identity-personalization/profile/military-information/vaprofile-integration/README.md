# Project outline: Integrate military information through VA Profile

**Last updated: January 11, 2023**

### Communications

- **Github labels**: vsa-authenticated-exp; profile
- **Slack channel**: [#accountexp-authexp](https://dsva.slack.com/channels/accountexp-authexp); [#va-profile](https://dsva.slack.com/channels/va-profile)

### Roles

#### OCTO team

[This is managed by the VA.gov profile team](https://github.com/department-of-veterans-affairs/va.gov-team/blob/master/products/identity-personalization/profile/README.md#roles).

#### Stakeholders

|Name|Role|Email|
|----|----|-----|
|Barb Burns| VA Profile |Barbara.Burns@va.gov |
|Tom Traficante| VA Profile |Thomas.Traficante@va.gov|
|Barry Willbanks| VA Profile |bwilbanks@meetveracity.com|
|Josh Lindsey| VA Profile|Joshua.Lindsey@va.gov|

### Table of Contents

- [Overview](#overview)
- [Problem Statement](#problem-statement)
- [User Outcomes](#user-outcomes)
- [Business Outcomes](#business-outcomes)
- [Measuring Success](#measuring-success)
- [Solution Approach](#solution-approach)

## Overview

In February 2022, we were informed that the service through which the VA.gov profile had historically (since 2018) received military information -- eMIS -- was being retired, and that military information was now fully integrated through the VA Profile backend. Our team needs to update the military information integration in the VA.gov profile to connect through VA Profile before the eMIS service is retired later in 2022.

## Problem Statement

- As a Veteran, I want to be able to see my service history when I log into VA.gov.

## User outcomes

### Desired User Outcomes

- There will be no interruptions in service, and people will be able to view their military information without issue as we transition between eMIS and VA Profile.

### Undesired User Outcomes

- There is downtime as a result of not getting this integration done on time.

## Business outcomes

### Desired Business Outcomes

- This is a win all around from a business perspective, as we already integrate much of our information through the VA Profile backend.

### Undesired Business Outcomes

- None

## Measuring success

### Objective: Update integration of military information to go through VA Profile.

[Military info analytics](https://analytics.google.com/analytics/web/?authuser=0#/dashboard/-x0K5pQPRTaQCa_WzXnEDg/a50123418w177519031p176188361/)

**KPI**: We see at least a 95% success rate for retrieving this information from the backend.

|Field name|% of successful retrievals in first month after launch: August 18 - September 17, 2022| Second month after launch: September 18 - October 17, 2022| Third month after launch: October 18 - November 17, 2022|
|----------|------------|------------|------------|
|Military information|100%|100%|100%|

## Solution approach

### Key dates

- April 2022: Begin migrating integration from eMIS to VA Profile.
- May  2022: Kickoff; Begin integration work.
- June 2022: QA
- July 2022: Waiting for VA Profile to give prod permissions
- August 2022: UAT
- August 9, 2022: Launch to 25%
- August 15, 2022: Launch to 50%
- August 18, 2022: Launch to 100%
