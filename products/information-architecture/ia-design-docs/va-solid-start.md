...
* It is part of a branded program known as "VA Solid Start" which has its own web pages in Google and in the email design (which is how users will access the URL to even get to this program to schedule an VASS call with the LoRota architecture) uses this language.
