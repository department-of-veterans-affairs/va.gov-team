...
Vet Pro is an application built by Panoramic Software, which is used by VSOs and Veterans to submit benefit claims. It includes a form search page which uses the LH forms API similarly to how the VA.gov search page does except that it is a little more integrated as a next step with submitting the forms for benefit claims immediately after. 
    - Note from Nick: It makes sense to me, but to restate, right now relying directly on the CMS for a data source is not technically feasible. If it were, we would've done it already (much simpler.) But in some of the restructuring we are considering in #content-release-engineering it could become possible
    - Note from Elijah Lynn: The CMS cannot technically accept any incoming public external traffic right now, because it is in a specific PROD network (vagov-prod vpc) that doesn't have an internet gateway (to allow the traffic in). We would need to move the CMS to a different VPC and change things up a bit, including public DNS and public TLS certificates (or passthrough with Lighthouse). That is non-trivial (not impossible though). A bunch more detail I haven't included, to keep this answer more brief. (edited) 
    - Additional note from Nick: but since I was already typing out :smile: - The CMS is not publicly exposed, as in, you can't navigate to https://prod.cms.va.gov/ from the open internet. It is VA intranet-only. This means that va.gov/find-forms can't connect to it at all.
    - Another note from Nick: if you mean "cache" the data from Drupal into a JSON file during the build then do only client-side searching, that's actually what we do for the Resources search app. But in the case of Find Forms, Lighthouse's API came before CMS had anything to do with forms (complicated data flow...complicated history too), so it wasn't even considered then bc it wasn't possible. It is possible now, but the JSON file would be very large, so possibly very slow on slow internet

## Find a Form POC
- Brian McCarthy's no longer our point of contact.
