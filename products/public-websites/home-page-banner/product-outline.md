...
- There are multiple issues related to this process:
  ...
  - (4) **Delayed publishing** - at one time, deploying the yaml file [was virtually real-time because content builds used to be faster] did not require waiting for an hourly content build. Once reviewed and approved by the PW FE engineer, the file could be pushed into production in real-time.  However, a new content build process introduced in mid-2020 moved the HP banner alert into the standard publishing path.  The business requirement is for the content to go live within 15 minutes of request for PR review.
