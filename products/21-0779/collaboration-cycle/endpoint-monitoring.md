# Endpoint Monitoring - Form 21-0779
...
This document outlines the endpoint monitoring strategy for Form 21-0779, ensuring compliance with VA.gov Platform QA Standard #6: Endpoint Monitoring.

**Key Architecture:** Form 21-0779 uses a shared Sidekiq job (`Lighthouse::SubmitBenefitsIntakeClaim`) that is also used by Forms 21-4192, 21P-530a, and approximately 19 other VA.gov forms. This enables efficient, unified monitoring across all forms using this infrastructure.

---

## Endpoints

### 1. POST /v0/form210779 (Submit Form)

**Purpose:** Accept and process nursing home information request submissions
...
```json
{
  "data": {
    ...
    "attributes": {
      ...
      "regional_office": [],
      "confirmation_number": "V-EBC-12346",
      "guid": "xyz789...",
      "form": "21-0779"
    }
  }
...
2. `SavedClaim::Form210779.new(form: payload.to_json)` created
3. Saved to database
...
### 2. POST /v0/form210779/download_pdf (Download PDF)

**Purpose:** Generate and return PDF version of nursing home information request
...
- Content-Disposition: `attachment; filename="21-0779_[UUID].pdf"`
- Body: PDF binary data

**Processing Flow:**
1. Controller receives form data
2. `PdfFill::Filler.fill_ancillary_form(parsed_form, uuid, '21-0779')` called
...
StatsD.increment("api.form210779.success")  # Successful save + job enqueue
StatsD.increment("api.form210779.failure")  # Validation or save failure
```

**Shared Job Metrics (covers Form 21-0779 and other forms using same job):**
```ruby
...
# => ClaimID=V-EBC-12346 Form=21-0779
...
**Key Traces:**
- `POST /v0/form210779` → SavedClaim creation → Sidekiq enqueue
- `POST /v0/form210779/download_pdf` → PDF generation
...
**Location:** Form 21-0779 has dedicated widgets within the unified dashboard covering all Aquia forms (21-4192, 21-2680, 21P-530a, 21-0779).

---

### Form 21-0779 Specific Widgets

#### Widget 1: Submission Status (Timeseries)
- **Metrics:** 
  - `sum:api.form210779.success{*}` (Successful Submissions)
  - `sum:api.form210779.failure{*}` (Failed Submissions)
- **Visualization:** Stacked area chart
- **Timeframe:** Past 24 hours
- **Purpose:** Track submission volume and failures

#### Widget 2: Error Rate (Query Value)
- **Metric:** `sum:api.form210779.failure{*} / (sum:api.form210779.success{*} + sum:api.form210779.failure{*}) * 100`
...
- **Metric:** `sum:api.form210779.success{*}.as_count()`
- **Timeframe:** Past 24 hours
- **Purpose:** Total successful submissions

#### Widget 4: Recent Errors (Log Stream)
- **Query:** `service:vets-api status:error "Form210779"`
- **Display:** Most recent 10 errors
- **Purpose:** Quick error triage

#### Widget 5: Request Latency (Timeseries)
- **Metric:** `avg:trace.rack.request.duration{resource_name:"POST /v0/form210779"}`
...
These widgets monitor the shared Sidekiq job used by Form 21-0779 and other forms:
...
### Form 21-0779 Specific Monitors

#### Monitor 1: Low Traffic Detection
- **Name:** `[Aquia] Form 21-0779 - Anomalous Traffic Drop`
- **Type:** Anomaly Detection
- **Metric:** `sum:api.form210779.success{*}.as_rate()`
...
- **Name:** `[Aquia] Form 21-0779 - PDF Generation Failures`
- **Type:** Log Alert
- **Query:** `logs("service:vets-api status:error \"Form210779\" \"PDF\"").index("*").rollup("count").last("15m") > 5`
- **Threshold:** > 5 errors in 15 minutes
- **Notification:** `#benefits-optimization-aquia`
- **Purpose:** Alert on PDF generation issues

#### Monitor 3: High 5xx Error Rate
- **Name:** `[Aquia] Form 21-0779 - High 5xx Error Rate`
- **Type:** Metric Alert
- **Metric:** `sum:trace.rack.request.errors{resource_name:"POST /v0/form210779",http.status_code:5*}.as_count()`
...
These monitors cover Form 21-0779 and other forms using the same infrastructure:
...
2. **Retrieve Claim Details**
   ...
   > SavedClaim::Form210779.find([claim_ids_from_query])
   ```
...
5. **Manual Remediation (if Lighthouse API is healthy)**
   ```ruby
   # Retrieve claim
   claim = SavedClaim::Form210779.find(claim_id)
...
1. **Verify PDF Template Exists**
   ```bash
   docker exec vets-api-web-1 ls -la lib/pdf_fill/forms/pdfs/21-0779.pdf
   ```
...
**Find all Form 21-0779 submissions in last 24 hours:**
```
service:vets-api "ClaimID=V-EBC-*" "Form=21-0779"
```

**Find all Form 21-0779 errors:**
```
service:vets-api status:error "Form210779"
```

**Find PDF generation errors:**
```
service:vets-api status:error "Form210779" "PDF"
```

**Find silent failures affecting Form 21-0779 (shared job):**
```
...
**Find recent Form 21-0779 submissions:**
```sql
...
**Find failed Form 21-0779 submission attempts:**
```sql
SELECT 
  fsa.id,
  fsa.saved_claim_id,
...
- [ ] StatsD metrics instrumented (`api.form210779.success`, `api.form210779.failure`)
- [ ] Rails logging in place (ClaimID, form type, errors)
- [ ] APM tracing enabled
- [ ] Dashboard widgets created for Form 21-0779
- [ ] 3 form-specific monitors created (traffic, PDF, 5xx)
- [ ] Shared monitors verified covering Form 21-0779
