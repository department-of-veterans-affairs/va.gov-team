...
***Is there a status for when an agent is currently reviewing a question? Or does it skip from `New` to an agent has responded?**
<br> ***Why is the status set to `In progress` when the agent responds?**
