...
|`Closed`|`Closed`|We closed this question after 60 days of no activity.|This isn't shown in the dashbaord but it is shown if someone 'Check[s] the status of their question.' The system automatically updates the status of `Solved` business inquiries to `Closed` after 60 days of no activity. This only applies to business inquiries, and happens so they will be removed from their dashboard. Personal and unauthenticated inquiries are never set to `Closed`.|
