# About this document

The CRM team requested this information from the line of business. We have moved it to Github for our notes.

<br>

# Health Care Write Up

### Submitted by: Michael Bingham on May 2, 2022

We are requesting the following from you (in the form of a Word document or email) so that we can begin to workshop a reference document for agents to assist in correctly routing.

#### 1. Provide any point(s) of contact who are responsible for process documentation and/or are subject matter experts in terms of routing and queues for your LoB:

- Timothy Lavery

#### 2. Provide a list of most common correctly routed topics in the form of submitter questions (submitter questions you are able to answer)
- All questions related to the VA Civilian Health and Medical Program (CHAMPVA) and CHAMPVA In-House Treatment Initiative (CITI).

#### 3. Provide a list of items that seem like they would belong to your LoB, but don’t (submitter questions you cannot answer). Ex: DMC does not handle medical debt/ payments for medical services
- The Specialty Team in the Family Member Customer Experience Center does not handle:
  - Assisted Living (IE… in nursing home, or needs assisted living benefits)
  - Survivor Benefits
  - IT Issues (for example, but not limited to ID.me or MyHealtheVet issues)
  - Questions from providers regarding eligibility, benefits, claims status, or claims filing.

---

# Caregiver Support Write Up

### Submitted by: Mary Ewers on May 2, 2022

We are requesting the following from you (in the form of a Word document or email) so that we can begin to workshop a reference document for agents to assist in correctly routing.

#### 1. Provide any point(s) of contact who are responsible for process documentation and/or are subject matter experts in terms of routing and queues for your LoB
- Ewers, Mary E Mary.Ewers@va.gov
- Reynolds, Christy F. Christy.Reynolds2@va.gov
- Mcalinden, Jennifer L. Jennifer.Mcalinden@va.gov
- Westerling, Laura Laura.Westerling@va.gov
- Sherman, Cynthia A Cynthia.Sherman@va.gov
- Farrell, Danielle (VCL) Danielle.Farrell@va.gov

#### 2. Provide a list of most common correctly routed topics in the form of submitter questions (submitter questions you can answer)
- General information related to caregiving and available supports and services
- Program of Comprehensive Assistance for Family Caregivers (PCAFC)
- Application, Eligibility, Services available, Appeal
- Program of General Caregiver Support Services (PGCSS)
- Eligibility, Services available
- Assistance navigating barriers to accessing care
- Supportive counseling for caregivers

#### 3. Provide a list of items that seem like they would belong to your LoB, but don’t (submitter questions you cannot answer). Ex: DMC does not handle medical debt/ payments for medical services
- Medical care questions or concerns (including prosthetics, record management, immunizations, scheduling of medical appointments, prescription refills)
- *Things that should be split to the local VAMC for follow up
- Complaints about a medical provider at a local VA
- *Things that should be split to the local VAMC for follow up
- Website access issues/technical issues& guidance
- Medical billing / Access to billing system
- Debts
- Agency Contracting – questions about starting a business/contracting with the VAMC
- Benefit questions – Service Connection, Aid and Attendance
- Visitation policies for local VAMC

---

# ECM Write Up

### Submitted by: Debra Hicks on April 26, 2022

We are requesting the following from you (in the form of a Word document or email) so that we can begin to workshop a reference document for agents to assist in correctly routing.

#### 1. Provide any point(s) of contact who are responsible for process documentation and/or are subject matter experts in terms of routing and queues for your LoB

No problem for AVA/ECM. We get the type of inquiries that we should get. Release 3 should take care of any issue with re-routing to the correct facility without going through many hoops. The re-routing inquiries we get typically do not come from Vets directly.

#### 2. Provide a list of most common correctly routed topics in the form of submitter questions (submitter questions you can answer)

We provide information straight out of KM articles. Most questions are regarding VA Healthcare. There is very little need for ad lib.

#### 3. Provide a list of items that seem like they would belong to your LoB, but don’t (submitter questions you cannot answer). Ex: DMC does not handle medical debt/ payments for medical services

This is not an issue for us. KM articles provide the needed responses to most inquiries. Agents just copy and paste  straight from KM artless to include POCs and phone numbers as needed.
