...
- **Objectives:** Ensure the form functions correctly, validates input, integrates properly with the backend, CRM and other APIs, and provides a smooth user experience with appropriate accessibility expectations.
...
Integration Testing
- **Decision:** Initial automated tests will be run locally in order to prevent blocking the pipeline. Once Authenticated Health - I am the Vet - Myself passes, we’ll promote it to the build pipeline. 
  - In Scope AT Clickable Prototype: Form Healthcare 
...
- Example Reference: Unauthenticated Life Insurance Cypress Test (Proof of Concept)
- Example Scenarios: 
...
_Manual Usability & Accessibility Testing_
- Scope: 
...
- Example: Navigate the form with the keyboard, visually inspect the form, and verify form responsiveness on different devices.
 - **Required tools:** JAWS (to gain access see Slack) 
 - **References:**  Prototype Figma workspace

| Accessibility Accommodation | Potential Tools |
| --------------------------- |-----------------|
| **Magnification Devices** | ZoomText, ZoomText Fusion, Magnifier (Windows), Zoom |
| **Alternative methods of navigation** | Mix of Switch system, Dragon NaturallySpeaking, Voice Control (Apple), One-handed keyboards, Head mouse, Sip and puff device |
| **Screen Readers** | Mix of NVDA, JAWS, VoiceOver (Apple), Talkback (Android) |

The research plan can be found here in github.  
### 4. Testing Team
...
|----------|-------------|--------------------|
| **Severity 1**| This is a top tier. Bugs must be corrected prior to Assistive Tech Study (AT) **Example:** Unable to populate VA Health Facility Location, Incorrect Progress Bar.
|Jumps backlog, potentially interrupt a developer |
| **Severity 2** | This will impact AT, but lesser priority | Give a heads up before picking up another ticket|
| **Severity 3** | Not blocking AT, but will be needed to be resolved prior to release **Example:** names not allowing apostrophe or hyphen |
Will be ordered in backlog accordingly


### Process for Bug Creation: 

When identifying a bug take the following steps: 
- Include steps to recreate the defect
- Include screenshots
- Keep comments about the bug in the ticket
- Tag Design (Tyler / Thomas)  
