...
6. `Form526Submission.user_uuid`: the unique identifier of the user that submitted the Form526. `user_uuid` is used as a key to delete the `InProgressForm` and the `IntentToFile`  in the `submit_form526_cleanup#perform` method. This "cleanup" method is called at the same time as the ancillary jobs. Prior to 06/27/2025 this one column in the database could be one of several different "uuid"s that a user is given on the platform: ID.me, Login.gov, DSLogon, or MHV. On 06/27/2025 [the `user_uuid` has been updated](https://github.com/department-of-veterans-affairs/va.gov-team/blob/master/products/identity/Products/user_uuid%20Refactor/user_uuid_refactor_announcement.md) to source from the vets-api's `UserAccount` table.
