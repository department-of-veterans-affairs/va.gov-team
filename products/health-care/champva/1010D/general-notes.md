...
- 3/4/24 - 10-10d usability testing will be done in staging. 10-7959f-1 testing - looking into UXPin (CodePen involves too much coding, Figma licenses are too limited - would require the VA to update, since Figma is new not a lot of precedent for testing).
...
- 3/6/24 - Recommendation: To avoid asking redundancy, confusion or displaying overlapping information during the experience of enrolling in CHAMPVA (with OHI) we should consolidate the 1010D and 7959c forms. We will still have a standalone 7959c form to capture any coverage/ address updates.
   - Question for CAIA: How may we solve this problem to best accommodate the PRA? (A: Per CAIA, we can make the recommendation since we are following the spirit of PRA - reduce burden time on user). Can we give 2 control numbers on the intro page if we consolidate. (Lois can help us find the contact in legal - this would go through General Council). 
