...
- There’s no consistently governed, patient friendly data for clinic name and physical location across VistA instances, VISNs, and clinics in order to provide Veterans accurate information on where to go once inside VA facilities, especially VAMC’s and hospitals, which has caused Veterans to, technically, "check in," but not be ready to be seen by their provider. E.g., delivered in appointment text reminders and within eCheck-in. 
