This document describes the recommended setup for doing fullstack development of VA.gov, with a secure tunnel connection to an upstream API environment such as MHV. As such, it is suitable for developing on a non-GFE.
...
## Setup Steps
<details>
  <summary><b>Option 1: Connecting via SSM Tunnel</b></summary>

### SSM Tunnel Setup Steps
...
1. Establish an MFA token in your shell. From the root of the devops repository, run:
    ```sh
    . ./utilities/issue_mfa.sh <Aws.Username> <2FA code>
    ```
    It should print output like "AWS Session credentials saved. Will expire in 12 hours".
...
2. From the same terminal, discover a forward proxy instance to tunnel to in whichever environment is desired, using:
   ```sh
   ./utilities/ssm.sh fwdproxy <dev|staging>
The command will print the available instances and prompt you to connect a shell session to one. You can Ctrl-C out of this command at this point as you don't need to connect, you just want to print the available instances. - Connecting can be useful for debugging connection issues or testing updated hosts or interfaces in the forward proxy. - utilities/ssm.sh runs aws ec2 describe-instances (using a docker image) with --filters and a --query to retrieve ec2 hostnames. 3. Run the following command to establish the SSM tunnel:
...
* `instance_id` is the AWS instance id (like `i-0a751576c718bf730`) from step 2.
...
Possible enhancement - a PR would be welcome for an ssm-tunnel.sh command that does the instance discovery and establishes the tunnel in one step, combining steps 2 and 3. Something like ssm-tunnel.sh <env> <local port> <remote port>
</details>
<details> <summary><b>Option 2: Connecting via SOCKS Proxy</b></summary>
SOCKS Proxy Setup Steps
...
vtk socks on
# Set up the SOCKS proxy tunnel. In this example, the local port is 2003 and the fwdproxy port is 4498
socat TCP-LISTEN:2003,fork SOCKS5:localhost:fwdproxy-staging.vfs.va.gov:4498,socksport=2001
# Validate the connection:
curl -k -H 'Host: fwdproxy-staging.vfs.va.gov' https://localhost:2003/api/v1
```
</details>
...
Update your local vets-api configuration by overriding the relevant values in config/settings.local.yml and restarting the Rails server. This example assumes you used 2003 as the local_port when setting up the tunnel above.
...
So, we want to configure vets-api to ignore hostname verification for outbound connections. But, we want to do that in a way that won't accidentally sneak into a pull request and make its way to our deployed environments. So we're going to add a config initializer and then set git up to ignore it.
Create a file `config/initializers/faraday_ssl_noverify.rb` with these contents:
```
# frozen_string_literal: true

Faraday.default_connection_options = { ssl: { verify: false } }
```
Append the following to `.git/info/exclude:`
```
