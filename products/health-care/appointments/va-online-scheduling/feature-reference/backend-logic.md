...
| [VA Video Care at Home](./appointment-types/va-video-care-at-home.md) |  `appointment.kind` = `telehealth`, and `telehealth` field should have `vvsKind` field equal to `MOBILE_ANY`  (i.e. `telehealth: {vvsKind: 'MOBILE_ANY/ADHOC'}`) and `Appointment.extension.patientHasMobileGfe === false` |
| [VA Video Care on GFE](./appointment-types/va-video-care-on-gfe.md) | `appointment.kind` = `telehealth`, and `telehealth` field should have `vvsKind` field equal to `MOBILE_ANY`  (i.e. `telehealth: {vvsKind: 'MOBILE_ANY/ADHOC'}`) and `Appointment.extension.patientHasMobileGfe === true` |
