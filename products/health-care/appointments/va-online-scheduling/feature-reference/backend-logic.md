...
| [VA Video Care at Home](./appointment-types/va-video-care-at-home.md) |  `telehealth` field should have `vvsKind` field equal to `MOBILE_ANY`  (i.e. `telehealth: {vvsKind: 'MOBILE_ANY'}`) and `Appointment.extension.patientHasMobileGfe === false` |
| [VA Video Care on GFE](./appointment-types/va-video-care-on-gfe.md) | `telehealth` field should have `vvsKind` field equal to `MOBILE_ANY`  (i.e. `telehealth: {vvsKind: 'MOBILE_ANY'}`) and `Appointment.extension.patientHasMobileGfe === true` |
