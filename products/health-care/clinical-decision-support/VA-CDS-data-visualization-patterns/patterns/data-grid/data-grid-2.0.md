...
| Table state  | exporting state  | As a Data Grid user, I need to access the current state of the table and return its value so that I can use it.  | P1  | In Progress  |
| Table state  | importing state  | As a Data Grid consumer, i need to restore the grid to a previous state so that I can return to the desired grid state.  | P1  | In Progress  |
| Selecting _one_ row  | client side selecting row  | As a data grid user, I need to be able to select one row so that I can interact with it  | P1  | Not Started  |
| Selecting _one_ row  | programmatically select a row  | As a data grid developer, I need to able to programmatically select a row so I can interact with it directly  | P1  | Not Started  |
| Selecting _multiple_ rows  | expand select row to select rows  | As a data grid user, I need to be able to select multiple rows so that I can interact with it  | P1 or P2  | Not Started  |
| Selecting _multiple_ rows  | accessing selected row(s) programmatically  | As a data grid developer, I need to able to programmatically select multiple rows so I can interact with it directly  | P1 or P2  | Not Started  |
| Edit row  | client side add edit  | As a data grid user, I need to be able to edit an entire row's data so that I can change all fields inside of a given row.  | P1  | Not Started  |
| Add a row  | client side add row  | As a data grid user, I want to add rows to the table so that I can add new information.  | P1  | Not Started  |
| Add a row  | programmatically insert row in the data set  | As a data grid developer, I want to programmatically add rows to the table so that I can interact with it directly.  | P1  | Not Started  |
| Delete a row  | client side delete row  | As a user of the data grid, I need to be able to delete a row so that I can delete existing data  | P1  | Not Started  |
| Filter  | accessing client side filter  | As a data grid developer, I need to be able to use filters function from the toolbar so that I can interact with the table  | P1  | Not Started  |
| Sorting  | custom sorting in the columns  |   | P2  | Not Started  |
| Managing errors UI  | showing errors UI   | As a data grid user that makes an incorrect input or interaction, I want to receive feedback so that I know to make the necessary update  | P1  | Not Started  |
...
| Managing the column definition  | grid options changes (changing column definition array on the fly)  |   | P2  | Out of scope  |
| Cell editor enhancements  | date picker cell editor  |   | P2  | Out of scope  |
| Cell editor enhancements  | add type checks to cell editor  |   | P3  | Out of scope  |
| Managing columns  | Client side moving column  | As a user of the data grid, I need to able able to drag and drop columns so I can reorganize columns in a different order  | P3  | Out of scope  |
...
| Header row  | sticky header as a default view  | As a data grid user, I want the header row to be always sticky so that I can reference it as I scroll vertically.  | P3  | Out of scope  |
