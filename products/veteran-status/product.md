...
> [!NOTE] 
> Regarding Eligibility: Only Veterans with an honorable or generally honorable code (A or B) are eligible for the card. The vetern status card is using the same military history as the profile service history, however there's an additional check for the discharge code. 
