# Endpoint Monitoring - Form 21P-530a
...
This document outlines the endpoint monitoring strategy for Form 21P-530a, ensuring compliance with VA.gov Platform QA Standard #6: Endpoint Monitoring.

**Key Architecture:** Form 21P-530a uses a shared Sidekiq job (`Lighthouse::SubmitBenefitsIntakeClaim`) that is also used by Forms 21-4192, 21-0779, and approximately 19 other VA.gov forms. This enables efficient, unified monitoring across all forms using this infrastructure.

---

## Endpoints

### 1. POST /v0/form21p530a (Submit Form)

**Purpose:** Accept and process burial allowance application submissions
...
2. `SavedClaim::Form21P530a.new(form: payload.to_json)` created
3. Saved to database
...
### 2. POST /v0/form21p530a/download_pdf (Download PDF)

**Purpose:** Generate and return PDF version of burial allowance application
...
- Content-Disposition: `attachment; filename="21P-530a_[UUID].pdf"`
- Body: PDF binary data

**Processing Flow:**
1. Controller receives form data
2. `PdfFill::Filler.fill_ancillary_form(parsed_form, uuid, '21P-530a')` called
...
StatsD.increment("api.form21p530a.success")  # Successful save + job enqueue
StatsD.increment("api.form21p530a.failure")  # Validation or save failure
```

**Shared Job Metrics (covers Form 21P-530a and other forms using same job):**
```ruby
...
# => ClaimID=V-EBC-12345 Form=21P-530a
...
**Key Traces:**
- `POST /v0/form21p530a` → SavedClaim creation → Sidekiq enqueue
- `POST /v0/form21p530a/download_pdf` → PDF generation
...
**Location:** Form 21P-530a has dedicated widgets within the unified dashboard covering all Aquia forms (21-4192, 21-2680, 21P-530a, 21-0779).

---

### Form 21P-530a Specific Widgets

#### Widget 1: Submission Status (Timeseries)
- **Metrics:** 
  - `sum:api.form21p530a.success{*}` (Successful Submissions)
  - `sum:api.form21p530a.failure{*}` (Failed Submissions)
- **Visualization:** Stacked area chart
- **Timeframe:** Past 24 hours
- **Purpose:** Track submission volume and failures

#### Widget 2: Error Rate (Query Value)
- **Metric:** `sum:api.form21p530a.failure{*} / (sum:api.form21p530a.success{*} + sum:api.form21p530a.failure{*}) * 100`
...
- **Metric:** `sum:api.form21p530a.success{*}.as_count()`
- **Timeframe:** Past 24 hours
- **Purpose:** Total successful submissions

#### Widget 4: Recent Errors (Log Stream)
- **Query:** `service:vets-api status:error "Form21P530a"`
- **Display:** Most recent 10 errors
- **Purpose:** Quick error triage

#### Widget 5: Request Latency (Timeseries)
- **Metric:** `avg:trace.rack.request.duration{resource_name:"POST /v0/form21p530a"}`
...
These widgets monitor the shared Sidekiq job used by Form 21P-530a and other forms:
...
### Form 21P-530a Specific Monitors

#### Monitor 1: Low Traffic Detection
- **Name:** `[Aquia] Form 21P-530a - Anomalous Traffic Drop`
- **Type:** Anomaly Detection
- **Metric:** `sum:api.form21p530a.success{*}.as_rate()`
...
- **Name:** `[Aquia] Form 21P-530a - PDF Generation Failures`
- **Type:** Log Alert
- **Query:** `logs("service:vets-api status:error \"Form21P530a\" \"PDF\"").index("*").rollup("count").last("15m") > 5`
- **Threshold:** > 5 errors in 15 minutes
- **Notification:** `#benefits-optimization-aquia`
- **Purpose:** Alert on PDF generation issues

#### Monitor 3: High 5xx Error Rate
- **Name:** `[Aquia] Form 21P-530a - High 5xx Error Rate`
- **Type:** Metric Alert
- **Metric:** `sum:trace.rack.request.errors{resource_name:"POST /v0/form21p530a",http.status_code:5*}.as_count()`
...
These monitors cover Form 21P-530a and other forms using the same infrastructure:
...
2. **Retrieve Claim Details**
   ...
   > SavedClaim::Form21P530a.find([claim_ids_from_query])
   ```
...
5. **Manual Remediation (if Lighthouse API is healthy)**
   ```ruby
   # Retrieve claim
   claim = SavedClaim::Form21P530a.find(claim_id)
...
1. **Verify PDF Template Exists**
   ```bash
   docker exec vets-api-web-1 ls -la lib/pdf_fill/forms/pdfs/21P-530a.pdf
   ```
...
**Find all Form 21P-530a submissions in last 24 hours:**
```
service:vets-api "ClaimID=V-EBC-*" "Form=21P-530a"
```

**Find all Form 21P-530a errors:**
```
service:vets-api status:error "Form21P530a"
```

**Find PDF generation errors:**
```
service:vets-api status:error "Form21P530a" "PDF"
```

**Find silent failures affecting Form 21P-530a (shared job):**
```
...
**Find recent Form 21P-530a submissions:**
```sql
...
**Find failed Form 21P-530a submission attempts:**
```sql
SELECT 
  fsa.id,
  fsa.saved_claim_id,
...
- [ ] StatsD metrics instrumented (`api.form21p530a.success`, `api.form21p530a.failure`)
- [ ] Rails logging in place (ClaimID, form type, errors)
- [ ] APM tracing enabled
- [ ] Dashboard widgets created for Form 21P-530a
- [ ] 3 form-specific monitors created (traffic, PDF, 5xx)
- [ ] Shared monitors verified covering Form 21P-530a
