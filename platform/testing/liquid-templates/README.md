...
The `document-title` check has been disabled because `liquid` template `include` files (not entire page templates) can also be tested by this tool and those files are missing the `title` tag (along with the `html`, `head`, and `body` sections, etc., etc., etc.).
