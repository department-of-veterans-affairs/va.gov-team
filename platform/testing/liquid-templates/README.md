...
The `CSS` file for the static pages found on VA.gov is generated during the build process, and therefore, cannot be referenced by the `HTML` files generated by this testing tool. Because of this, accessibility checks for things that require the `CSS` file, such as color-contrast and  visability checks, cannot be performed. We have configured `axeCheck()` to only check for accessibility violations that can be detected on the `HTML` documents generated by this tool (i.e. checks that don't require `CSS`).
