...
In `vets-api`, a VSA team can be assigned as a code owner of [module folders](https://github.com/department-of-veterans-affairs/vets-api/tree/master/modules). **This will remove `backend-review-group` as a required reviewer when changes are made to those folders.** Follow the conventions in the [`CODEOWNERS` configuration file](https://github.com/department-of-veterans-affairs/vets-api/blob/master/.github/CODEOWNERS) to assign your team to an application folder.
...
- The VSP teams approve changes to the `CODEOWNERS` configuration. The relevant VSP team will add new GitHub teams created by VSA teams to the applicable repository during this review since this is required for code owners rules to trigger.
