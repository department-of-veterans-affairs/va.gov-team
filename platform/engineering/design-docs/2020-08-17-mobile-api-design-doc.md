...
(such as MVI, Claims, Appointments), the mobile API will be built inside of the vets-api codebase, and exposed in the same 
...
while in other cases the mobile API will provide its own presentation layer on top of the 
...
modifications are required, changes will be made in a way to not break existing clients. The team is considering 
...
Enabling the mobile API will expose some tech debt in the `ApplicationController` that is not a requirement to fix; but it would clean up things for all its sub-classes.
The logic in the core `ApplicationController` that does not apply globally could move to concerns be and 
...
Some endpoints such as facilities may not require optimization. They will pass-through via routes:
...
or over-deliver, should implement fieldsets and includes in REST url params so no extra data is returned. JSON-API defines
