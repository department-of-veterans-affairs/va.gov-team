...
TODO - this sort of got covered by the "Work Estimates" section

### Code Location
All code will live in the [`devops` repo](https://github.com/department-of-veterans-affairs/devops)

### Testing Plan
We will stand up Sentry 10 in parallel with our existing Sentry 9.1.2. We will then configure the `Settings.sentry.dsn` of `dev` and/or `staging` to use the Sentry 10 deployment.
...
Create the necessary terraform code to build the AWS infrastructure needed for deploying into our `vagov-utility` VPC.
...
In the near future, this would be using redis elasticache & postgres RDS instead of the redis and postgres images in the docker-compose.yml.
