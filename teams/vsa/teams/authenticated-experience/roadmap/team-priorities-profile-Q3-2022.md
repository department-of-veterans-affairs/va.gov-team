...
This is some of the most important work we'll do this summer. My goal is that we have thorough documentation for the entire profile by the end of the quarter. Liz has already done a fantastic job of documenting the direct deposit functionality, but this work is too big to just fall on the designers on our team. We'll need everyone to participate so we have clear and easy-to-update documentation for all our features going forward, so that no knowledge lives solely in anyone's heads.

### Accessibility audit & updates

We should consolidate all accessibility feedback we've gotten, review it, and determine next steps. Tasks run from full-on projects (eg. moving edit from in-line to another page) to smaller tasks we can knock out as time allows.

### Forms system updates

A new version of the forms system is ready, so we need to update the profile so that it's using this new version. This might also help us with some accessibility wins!
