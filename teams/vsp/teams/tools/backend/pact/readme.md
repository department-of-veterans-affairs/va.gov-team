...
#### Authorization
If an authorized vets-api user is needed for a request to an endpoint, a helper method can be used to stub the session. Call the `build_user_and_stub_session`  method if your endpoint requires an authorized (signed in) user. See this [forms example](). TODO^^^^
