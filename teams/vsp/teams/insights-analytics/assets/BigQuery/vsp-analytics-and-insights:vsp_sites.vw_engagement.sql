...
FROM `vsp-analytics-and-insights.176188361.ga_sessions_*` AS ga,
UNNEST(ga.hits) AS hits
left join time_on_page top on top.date = PARSE_DATE("%Y%m%d", ga.date) 
                          AND top.user = ga.fullVisitorId and top.session = CONCAT(ga.fullVisitorId, ga.visitStartTime) 
