/***************************************************************
Name:         vsp_sites.vw_forms_funnels_vaos
Created:      2021-08-02
Created By:   Jason Cavnar
Description:  Standardized query for all form funnels
      ...
      FROM
          (
             ...
             INNER JOIN `vsp-analytics-and-insights.vsp_sites.forms_vaos` AS fv ON (hits.page.pagePath LIKE CONCAT('%',fv.step_link,'%') 
              ...
              WHERE
                _TABLE_SUFFIX between '20190618' and FORMAT_DATE('%Y%m%d', DATE_TRUNC(CURRENT_DATE(), WEEK(SATURDAY)))
