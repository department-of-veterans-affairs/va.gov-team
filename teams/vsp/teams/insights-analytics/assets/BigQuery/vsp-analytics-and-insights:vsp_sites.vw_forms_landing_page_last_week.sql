...
WHERE _table_suffix between FORMAT_DATE('%Y%m%d', DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 6 DAY), WEEK(SUNDAY))) 
...
INNER JOIN (SELECT DISTINCT form_name, landing_page FROM `vsp-analytics-and-insights.vsp_sites.forms` WHERE active = true) AS f ON hits.page.pagePath LIKE CONCAT('%', f.landing_page, '%')                         
WHERE _table_suffix between FORMAT_DATE('%Y%m%d', DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 6 DAY), WEEK(SUNDAY))) 
                        AND FORMAT_DATE('%Y%m%d', DATE_TRUNC(CURRENT_DATE(), WEEK(SATURDAY)))
